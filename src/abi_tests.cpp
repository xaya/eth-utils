...
TEST_F (AbiDecoderTests, DecodeMoveEvent)
{
  ...
  EXPECT_EQ (dec.GetAllDataRead (), data);
}

TEST_F (AbiDecoderTests, DecodeStringArray)
{
  /* This is (["foo", "bar"]) encoded using the eth_abi Python module as
     types (string[]).  */
  const std::string data = "0x"
  ...
  {
    size_t len;
    AbiDecoder sub = dec.ReadArray (len);
    ASSERT_EQ (len, 2);
    ASSERT_EQ (sub.ReadString (), "foo");
    ASSERT_EQ (sub.ReadString (), "bar");
  }
